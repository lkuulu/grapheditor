...
(function($) {
	 ...
	 */
	$.fn.jqPropertyGrid = function(obj, meta, updateCallback) {
    ...
    update = updateCallback;
		...
		innerHTML += '<tr class="pgTitleRow"><td colspan="2" class="pgGroupCell" ><img src="images/property.png">Property Editor</td></tr>';
	...
	function getPropertyRowHtml(pgId, name, value, meta, postCreateInitFuncs, getValueFuncs) {
		...
		if (type === 'boolean' || (type === '' && typeof value === 'boolean')) {
			...
			if (postCreateInitFuncs) { postCreateInitFuncs.push( callBackValueCB('click', elemId, name, value, meta.options)); }
		...
		// If options create drop-down list
			...
			if (postCreateInitFuncs) { postCreateInitFuncs.push( callBackValue('change', elemId, name, value, meta.options)); }
      ...
      valueHTML = '<textarea row=3 id="' + elemId + '" style="width:100%">' + value + '</textarea>';
		...
		// Default is textbox
			...
			//if (postCreateInitFuncs) { postCreateInitFuncs.push( updownIt( elemId ) ); }
 ...
 function updownIt(id) {
  	setTimeout(
  	  function() {
        $('#'+id).updown( 
          {step: 10,shiftStep: 100} 
        );
      },50);
  } 
  
  function callBackValueCB(event, id, name, value, options)  {
  	setTimeout(
  	   function() {
  	   	   $('#'+id).on(event, function() {
