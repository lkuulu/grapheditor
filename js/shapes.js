...
function CanvasState(canvas) {
  ...
  canvas.addEventListener('mousemove', function(e) {
    ...
    if ((myState.isSelectDrag) && (myState.selectFrame!=null)) {
  ...
  canvas.addEventListener('mouseup', function(e) {
    ...
    if (myState.isLinkDrag) {
        var upShape = null;
    	// si le link est déjà existant, on le supprime.
      for (var i = myState.shapes.length-1; i >= 0; i--) {
        if (myState.shapes[i].contains(mx, my)) {
    	...
    	if (myState.valid) {
      	// si on est sur un Link <> link en cours, on créer le link
      	for (var i = myState.shapes.length-1; i >= 0; i--) {
      	  if (myState.shapes[i].contains(mx, my)) {
      	    upShape = myState.shapes[i];
